    ...
    <Card>
      <CardHeader className="flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <span
            className="rounded-full border border-border/80 bg-background/40 px-3 py-1 text-xs text-muted-foreground"
            data-testid="historian-source-badge"
          >
      ...
      <CardContent>
        {usingFallback ? (
          <div className="mb-4 rounded-2xl border border-warning/30 bg-warning/10 p-4 text-sm text-muted-foreground">
            No history samples are available yet, so this chart is showing an explicitly labelled
            static demo curve.
          </div>
        ) : null}
        ...
        <div
          className="h-[380px] rounded-2xl border border-border/60 bg-surface-subtle/60 p-3"
          ...
          <ResponsiveContainer width="100%" height="100%">
            <RechartsLineChart data={chartData} margin={{ left: 0, right: 16 }}>
              ...
              <XAxis
                ...
                tickMargin={10}
              />
              <YAxis stroke={chartTheme.axis} tickLine={false} axisLine={false} tickMargin={10} />
              <Tooltip
                contentStyle={{
