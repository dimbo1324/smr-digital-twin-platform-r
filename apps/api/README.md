...
- simulation-only report export endpoint for JSON/CSV/PDF summaries with presentation template/section options
- scenario YAML draft validation endpoint that does not persist, deploy, or execute drafts
...
The OpenAPI contract is currently used for documentation, generated frontend TypeScript types, a lightweight generated Go DTO/client-helper baseline, frontend dev/test runtime validation, and CI drift checks. This service does not yet use generated Go server stubs or Go runtime validation from JSON Schema.
...
curl "http://localhost:8080/api/v1/reports/simulation-summary?window=1h&template=pid-control-review&sections=metadata,pidSummary"
...
The JSON response uses the normal API envelope. CSV responses use `text/csv` with `section,key,value,unit,source` rows. PDF responses use `application/pdf` with a compact human-readable summary generated by the API gateway. `template`, `sections`, and `includeDisclaimers` query options customize presentation only; mandatory simulation-only and non-regulatory wording remains present. Reports include `simulationOnly: true` and a disclaimer because they are demo summaries only, not regulatory reports, production audit exports, or nuclear compliance artifacts.

### `POST /api/v1/scenarios/validate`

Validates a YAML scenario draft for the Scenario Authoring UI:
...
This endpoint returns validation errors and warnings only. It does not persist drafts, write files, mutate the embedded scenario registry, deploy scenarios to the runtime, or control any real plant.
